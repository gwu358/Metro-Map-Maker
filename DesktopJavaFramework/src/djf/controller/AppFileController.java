...
//        gui.updateToolbarControls(saved);
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
		...
		dialog.show(props.getProperty(NEW_COMPLETED_TITLE), props.getProperty(NEW_COMPLETED_MESSAGE));
                app.getDataComponent().resetJTPS();
            }
	...
	saved = true;
        ...
        if(gui.savedFiles.contains(selectedFile.getName())){
            gui.savedFiles.remove(selectedFile.getName());
        }
        gui.savedFiles.add(selectedFile.getName());
        PrintWriter writer = new PrintWriter("recent.txt");
        for(String key:gui.savedFiles){           
            writer.println(key);
        }
        writer.close();
    ...
    public void handleExportRequest(){
        ...
        try {
            ...
            if (selectedFile != null) {
                ...
                String fileName=selectedFile.getPath().substring(selectedFile.getPath().lastIndexOf('\\')+1);
                new File(selectedFile.getPath()).mkdirs();
                app.getFileComponent().exportData(app.getDataComponent(), path+fileName);
                AppMessageDialogSingleton dialog = AppMessageDialogSingleton.getSingleton();
                dialog.show("Export Completed", "Files are successfully exported");
